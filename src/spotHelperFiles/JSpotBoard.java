...
public class JSpotBoard extends JPanel implements SpotBoard {
	...
	private static final Color DEFAULT_BACKGROUND_LIGHT = new Color(0.8f, 0.8f,
			0.8f);
	private static final Color DEFAULT_BACKGROUND_DARK = new Color(0.5f, 0.5f,
			0.5f);
	...
	public JSpotBoard(int width, int height, Color background) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				_spots[x][y] = new JSpot(bg, DEFAULT_SPOT_COLOR,
	...
	public void addSpotListener(SpotListener spot_listener) {
		/*
		 * for (int x = 0; x < getSpotWidth(); x++) { for (int y = 0; y <
		 * getSpotHeight(); y++) { _spots[x][y].addSpotListener(spot_listener);
		 * } }
		 */
		for (Spot s : this) {
	...
	public void removeSpotListener(SpotListener spot_listener) {
		for (Spot s : this) {
			s.removeSpotListener(spot_listener);
		}
	}
